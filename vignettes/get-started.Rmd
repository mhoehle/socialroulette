...
socialroulette:::partitions_to_distance(frame2, past_partitions)
```
...
For a more realist scenario, we pretend that each previous partition has been saved
...
socialroulette:::partitions_to_pairs( past_partitions ) %>% 
  group_split(date) %>%
  walk(~  write_csv(x=.x, path=file.path(temp_dir, stringr::str_c("socialroulette-", .$date[1], ".csv"))))
```
...
list(partition) %>% 
  ...
  write_csv(path=file.path(temp_dir, stringr::str_c("socialroulette-", .$date[1],".csv")))
```
...
partition %>% 
  ...
  select(room, email) %>% 
  write_csv(path=file.path(temp_dir, stringr::str_c("zoom-breakoutrooms.csv")))
```
